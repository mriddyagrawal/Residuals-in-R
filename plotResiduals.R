histoqq <- function(data, color = FALSE, square=FALSE, dist = "norm", height.scale = 1, ...) {
  require(tidyverse)
  ...
  if (!exists(q.name, mode = "function") || !exists(d.name, mode = "function")) {
    stop(paste0("Invalid distribution specified. Could not find functions '", q.name, "()' or '", 
                d.name, "()'. Please check your specification and ensure any necessary libraries are loaded."))
  }
  ...
  if(npass > length(data)){
    warning(paste0("There were ", (npass - length(data)), " missing observation(s). We removed these to make the plot."))
  }
  ...
  if (length(user.params) == 0) {
    if (dist == "unif") { # not well supported
      x1 <- min(df$x, na.rm=TRUE)
      xn <- max(df$x, na.rm=TRUE)
      user.params <- list(min = x1 - (xn-x1)/(nrow(df)-1), # UMVUEs
                          max = xn + (xn-x1)/(nrow(df)-1))
      message("Automatically estimated parameters for unif distribution: min=", round(user.params$min, 3), ", max=", round(user.params$max, 3),
              " using UMVUEs")
    } else if(dist == "binom"){ # not well supported
      ...
      message("Automatically estimated parameters for binom distribution: size=", est.size, 
              ", prob=", round(est.prob, 3), " (Used: ", estimation.method, ")")
    } else {
      fit.map <- c("norm" = "normal", "lnorm" = "log-normal", "exp" = "exponential", 
                   "logis" = "logistic", "geom" = "geometric", "nbinom" = "negative binomial", 
                   "pois" = "Poisson")
      ...
      tryCatch({
        ...
        message("Automatically estimated parameters for ", dist, " distribution: ", 
                paste(names(user.params), round(unlist(user.params), 3), sep = "=", collapse = ", "))
      }, error = function(e) {
        warning("MASS::fitdistr could not automatically estimate parameters for '", dist, 
                "'. Please check your specification and ensure any necessary libraries are loaded. Error: ", e$message)
      })
    }
  }
  
  # Create functions with proper parameters to use elsewhere
  qdist.w <- function(p) { do.call(qdist, c(list(p), user.params)) }
  ddist.w <- function(x) { do.call(ddist, c(list(x), user.params)) }
  ...
  # Specify bins
  bin.min <- if(is.finite(lower.bound)) lower.bound else min(df$x, na.rm=TRUE)
  bin.max <- if(is.finite(upper.bound)) upper.bound else max(df$x, na.rm=TRUE)
  ...
  plot.min <- max(min(c(df$x, qdist.w(c(0.001))), na.rm=T) - bin.width, lower.bound, na.rm=TRUE)
  plot.max <- min(max(c(df$x, qdist.w(c(0.999))), na.rm=T) + bin.width, upper.bound, na.rm=TRUE)
  
  if(is.discrete) {
    plot.range <- seq(ceiling(plot.min), floor(plot.max), by = 1)
  } else {
    base.seq <- seq(plot.min, plot.max, length.out = 1000)
    if(is.finite(lower.bound) && lower.bound >= plot.min && lower.bound <= plot.max) {
      base.seq <- c(base.seq, lower.bound - 1e-9, lower.bound, lower.bound + 1e-9)
    }
    if(is.finite(upper.bound) && upper.bound >= plot.min && upper.bound <= plot.max) {
      base.seq <- c(base.seq, upper.bound - 1e-9, upper.bound, upper.bound + 1e-9)
    }
  ...
  theoretical.density <- data.frame(val = plot.range) |>
    ...
    mutate(
      ...
      x.base = (val - intercept) / slope - (if(is.discrete) 0.1 * vx else 0),
      y.base = val - (if(is.discrete) 0.1 * vy else 0),
      x = x.base + h * nx,
      y = y.base + h * ny
    )
  ...
  all.coords.y <- c(df$x, hist.polygons$final.y, theoretical.density$y, kde.curve$y)
  ...
  if(is.discrete){
    histoqq.plot <- histoqq.plot +
      ...
      geom_segment(
        ...
        aes(x = x.base, y = y.base, xend = x, yend = y, linetype = "Theoretical")
      ) +
      # Empirical Density
      geom_segment(
        data = kde.curve,
  ...
  histoqq.plot <- histoqq.plot + 
    ...
    stat_qq_line(
      aes(sample = x),
      distribution = qdist, dparams = user.params, 
      color = "red",
      linewidth = 1,
      alpha = 0.75
    ) +
...
plotResiduals <- function(mod, color = FALSE, square = FALSE) {
  require(tidyverse)
  require(patchwork)
  ggdat <- data.frame(
    y.hat = fitted(mod),
  ...
  ####################################
  p1 <- histoqq(residuals(mod), color = color, square=square, dist = "norm", mean=0, sd=summary(mod)$sigma)

  ####################################
  # Create Data for Fitted vs Residual Plot"
  ####################################
  ggdat <- data.frame(x = fitted(mod), e = residuals(mod)) %>%
    mutate(id = 1:n())
  ...
  ####################################
  # Create Obs ID vs Residual Plot"
  ####################################
  p2 <- ggplot(data = ggdat, aes(x = id, y = e)) +
    geom_point(shape = 1) +
    ...
    theme_bw() +
    geom_hline(
      yintercept = c(-3, 3) * summary(mod)$sigma,
      color = "red",
      linetype = "dotted",
      linewidth = 0.75
    ) +
  ...
  ####################################
  p3 <- ggplot(data = ggdat, aes(x = x, y = e)) +
    geom_point(shape = 1) +
    geom_hline(yintercept = 0, color = "red", linetype = "dashed") +
  ...
  # Print Plots"
  ####################################
  (p1 + theme(legend.position = "bottom")) + p3
}
